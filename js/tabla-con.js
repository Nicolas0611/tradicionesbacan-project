...
function llenartabla(){
    db.collection('Contactenos').onSnapshot((snapshot) => {
        tabla.innerHTML = '';
...
function Borrarprod(IDborrar){
    if(IDborrar != undefined){
        TConf.innerHTML = '';
        TConf.innerHTML = `Confirmar consulta a borrar: ${IDborrar}`
        BConfir.innerHTML = '';
        BConfir.innerHTML =`Va a borrar la consulta con id: ${IDborrar}<br>Recuerde que la cotizacion se eliminara de forma definitiva
...
function borrardefirebase(IDborrar){
    db.collection("Contactenos").doc(IDborrar).delete().then(() => {
